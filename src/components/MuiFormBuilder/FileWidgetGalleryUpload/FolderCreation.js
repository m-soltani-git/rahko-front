...
export default function FolderCreation({ path, reload }) {
  const [name, setName] = useState('');
  const [open, setOpen] = useState(false);
  const { userToken } = useSelector((state) => state.auth);

  const [creation, { loading }] = useMutation(graph.create.query, {
    context: {
  ...
  return (
    <div>
      ...
      <Dialog dir="rtl" open={open} onClose={handleClose} fullWidth>
        <DialogTitle sx={{ px: 1 }}>ایجاد پوشه جدید</DialogTitle>
